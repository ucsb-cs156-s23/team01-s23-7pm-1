...
describe("SchoolTable tests", () => {
  ...
  test("Has the expected column headers, content and buttons", () => {
    ...
    expect(screen.getByTestId(`${testId}-cell-row-0-col-name`)).toHaveTextContent("Isla Vista Elementary School");

    expect(screen.getByTestId(`${testId}-cell-row-1-col-id`)).toHaveTextContent("3");
    expect(screen.getByTestId(`${testId}-cell-row-1-col-name`)).toHaveTextContent("Dos Pueblos High School");
  ...
  test("Has the expected column headers, content and no buttons when showButtons=false", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <SchoolTable schools={schoolFixtures.threeSchools} showButtons={false} />
        </MemoryRouter>
      </QueryClientProvider>
    );
  ...
  test("Delete button calls delete callback", async () => {
    ...
    fireEvent.click(deleteButton);
     ...
     const message = console.log.mock.calls[0][0];
     const expectedMessage = `deleteCallback: {"id":2,"name":"Isla Vista Elementary School","district":"Goleta Union School District","grade range":"K-6"})`;
     expect(message).toMatch(expectedMessage);
     restoreConsole();
  });
});
